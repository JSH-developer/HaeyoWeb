...
<link rel="stylesheet" href="../css/read_gal.css">
<%
	...
	fdao.countRead("gal",idx);
	
	String standard=request.getParameter("standard");
	String scontent=request.getParameter("scontent");
	
	String file =fdao.getFileFromIdx("gal", idx);
...
$(function(){
	...
	//현재 리스트 표시
	$("#gal<%=idx%>").css("background-color","#ebd5f5");
...
<table class="rf">
<%
	out.print(fdao.readContent("gal",idx));
%>
<tr><td><a class="blike"><img src='../images/smile.png' class='sicon' width='25' height='25'></a> <%=fdao.countBoardLike("gal", idx) %>  <a class="bunlike"><img src='../images/sad.png' class='sicon' width='25' height='25'></a> <%=fdao.countBoardUnlike("gal", idx) %></td></tr>
</table>

<!-- 버튼 모음 -->
<button onclick="location.href='searchBoard.jsp?page=<%=bpage%>&standard=<%=standard%>&scontent=<%=scontent%>'">목록가기</button>
<%  if( fdao.getIdFromIdx("gal", idx).equals(id) ){%>
	<button onclick="location.href='updateForm.jsp?page=<%=bpage%>&idx=<%=idx%>&standard=<%=standard%>&scontent=<%=scontent%>'">코멘트수정</button>
...
// 댓글
	List<ReplyBean> rbs = rdao.readReply("gal",idx);
	for(int i=rbs.size()-1; i>=0; i--){
		...
		out.println("<div class='rp_like'><a class='rlike'><img src='../images/smile.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyLike("gal", rbs.get(i).getIdx()) +" <a class='runlike' ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyUnlike("gal", rbs.get(i).getIdx()) +"</div><br>");
...
// 		대댓글을 입력하는 폼
		...
		//for문을 사용하여 대댓글 출력!
				List<RereplyBean> rrbs = rrdao.readRereply("gal",ridx);
...
<!-- 페이지에 해당하는 목록 불러오기 -->
<%
	out.print(fdao.searchgalBoard(bpage, standard, scontent));	
%>
<br>
<!-- 검색 -->
...
<%
	int cidx=0;
	cidx = fdao.countsearchIdx("gal", standard, scontent);	
