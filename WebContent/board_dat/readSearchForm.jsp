...
<link rel="stylesheet" href="../css/read_dat.css">
<%
...
<table class="rf">
<%
	out.print(fdao.readContent("dat",idx));
%>
<tr><td><a class="blike"><img src='../images/smile.png' class='sicon' width='25' height='25'></a> <%=fdao.countBoardLike("dat", idx) %>  <a class="bunlike"><img src='../images/sad.png' class='sicon' width='25' height='25'></a> <%=fdao.countBoardUnlike("dat", idx) %></td></tr>
</table>

<!-- 버튼 모음 -->
<button onclick="location.href='searchBoard.jsp?page=<%=bpage%>&standard=<%=standard%>&scontent=<%=scontent%>'">목록가기</button>
<%  if( fdao.getIdFromIdx("dat", idx).equals(id) ){%>
...
// 댓글
	List<ReplyBean> rbs = rdao.readReply("dat",idx);
	for(int i=rbs.size()-1; i>=0; i--){
		...
		out.println("<div class='rp_like'><a class='rlike'><img src='../images/smile.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyLike("dat", rbs.get(i).getIdx()) +" <a class='runlike' ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyUnlike("dat", rbs.get(i).getIdx()) +"</div><br>");
...
// 		대댓글을 입력하는 폼
		...
		//for문을 사용하여 대댓글 출력!
				List<RereplyBean> rrbs = rrdao.readRereply("dat",ridx);
...
<%
	out.print(fdao.searchdatBoard(bpage,standard,scontent));
%>
</table>
<!-- 검색 -->
...
<%
	int cidx=0;
	cidx = fdao.countsearchIdx("dat", standard, scontent);	
