...
<!-- 버튼 모음 -->
<button onclick="location.href='searchBoard.jsp?page=<%=bpage%>&standard=<%=standard%>&scontent=<%=scontent%>'">목록가기</button> 
<%  if( bdao.getIdFromIdx("kk", idx).equals(id) ){%>
	<button onclick="location.href='updateForm.jsp?page=<%=bpage%>&idx=<%=idx%>&standard=<%=standard%>&scontent=<%=scontent%>'">수정하기</button> 
...
<%
	out.print(bdao.categoryBoard("kk",bpage,"공지"));
	out.print(bdao.searchBoard("kk",bpage,standard,scontent));
%>
</table>
<!-- 검색 -->
...
<%
	int cidx=0;
	cidx = bdao.countsearchIdx("kk", standard, scontent);	
