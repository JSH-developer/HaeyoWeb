...
<link rel="stylesheet" href="../css/list_kk.css">
<link rel="stylesheet" href="../css/read_kk.css">
<%
...
$(function(){
	...
	//현재 리스트 표시
	$("#tr<%=idx%>").css("background-color","#f7e1e5");
...
<aside>
	<h1>골라보기</h1>
    	<a href="kkBoard.jsp?page=1"><button class="cate">전체보기</button></a><br>
    	<a href="kkBoard.jsp?page=1&category=잡담"><button class="cate">잡답해요</button></a><br>
    	<a href="kkBoard.jsp?page=1&category=모임"><button class="cate" >같이해요</button></a><br>
      	<a href="kkBoard.jsp?page=1&category=홍보"><button class="cate">홍보해요</button></a><br>
      	<a href="kkBoard.jsp?page=1&category=거래"><button class="cate" >거래해요</button></a><br>
      	<a href="kkBoard.jsp?page=1&category=베스트"><button class="cate">베스트</button></a><br>
</aside>
<article>
<table class="rf">
<%
	out.print(bdao.readContent("kk",idx));
%>
<tr><td><a class="blike" ><img src='../images/smile.png' class='sicon' width='25' height='25'></a> <%=bdao.countBoardLike("kk", idx) %>  <a class="bunlike" ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> <%=bdao.countBoardUnlike("kk", idx) %></td></tr>
</table>

<!-- 버튼 모음 -->
<button onclick="location.href='kkBoard.jsp?page=<%=bpage%>'">목록가기</button>  
<%  if( bdao.getIdFromIdx("kk", idx).equals(id) ){%>
...
// 댓글
	List<ReplyBean> rbs = rdao.readReply("kk",idx);
	for(int i=rbs.size()-1; i>=0; i--){
		...
		out.println("<div class='rp_like'><a class='rlike'><img src='../images/smile.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyLike("kk", rbs.get(i).getIdx()) +" <a class='runlike' ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyUnlike("kk", rbs.get(i).getIdx()) +"</div><br>");
...
// 		대댓글을 입력하는 폼
		...
		//for문을 사용하여 대댓글 출력!
				List<RereplyBean> rrbs = rrdao.readRereply("kk",ridx);
...
<%
	if(bcategory!=null){
		out.print(bdao.categoryBoard("kk",bpage,bcategory));
	}else{
		out.print(bdao.readBoard("kk",bpage));	
	}
%>
</table>
...
<%
	int cidx=0;
	if(bcategory != null){
		cidx = bdao.countCategoryIdx("kk", bcategory);
	}else{
		cidx = bdao.countIdx("kk");	
	}
	...
	if(bpage<10){ // 10이상부터 형식이 바뀜
		if(page_size<=10){
			for(int i=page_size-1;i>=0;i--){
				if(bcategory != null){
					out.println("<a href='kkBoard.jsp?page="+pages.get(i)+"&category="+bcategory+"' id='p"+pages.get(i)+"'>"+pages.get(i)+"</a>");
				}else{
					out.println("<a href='kkBoard.jsp?page="+pages.get(i)+"' id='p"+pages.get(i)+"'>"+pages.get(i)+"</a>");
				}
			}
		}else{
			...
			if(bcategory != null){
				out.println("<a href='kkBoard.jsp?page="+(page_size)+"&category="+bcategory+"' id='p"+(page_size)+"'>"+(page_size)+"</a>");
			}else{
				out.println("<a href='kkBoard.jsp?page="+(page_size)+"' id='p"+(page_size)+"'>"+(page_size)+"</a>");
			}
			
			if(bcategory != null){
				out.println("<a href='kkBoard.jsp?page="+( ((bpage+5)>=page_size) ? page_size:bpage+5 )+"&category="+bcategory+"'>  다음-> </a>");
			}else{
				out.println("<a href='kkBoard.jsp?page="+( ((bpage+5)>=page_size) ? page_size:bpage+5 )+"'>  다음-> </a>");
			}
		...
		if(bcategory != null){
			out.println("<a href='kkBoard.jsp?page="+(bpage-5)+"&category="+bcategory+"'> <-이전  </a>");
		}else{
			out.println("<a href='kkBoard.jsp?page="+(bpage-5)+"'> <-이전  </a>");
		}
		if(bcategory != null){
			out.println("<a href='kkBoard.jsp?page=1&category="+bcategory+"' id='p1'>1</a>");
		}else{
			out.println("<a href='kkBoard.jsp?page=1 id='p1'>1</a>");
		}
