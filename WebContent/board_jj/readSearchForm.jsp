...
<link rel="stylesheet" href="../css/read_jj.css">
<%
...
$(function(){
	...
	//현재 리스트 표시
	$("#tr<%=idx%>").css("background-color","#e4f5ef");
...
<table class="rf">
<%
	out.print(bdao.readContent("jj",idx));
%>
...
<%  if( bdao.getIdFromIdx("jj", idx).equals(id) ){%>
...
//댓글
	List<ReplyBean> rbs = rdao.readReply("jj",idx);
	for(int i=rbs.size()-1; i>=0; i--){
		...
		out.println("<div class='rp_like'><a class='rlike'><img src='../images/smile.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyLike("jj", rbs.get(i).getIdx()) +" <a class='runlike' ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyUnlike("jj", rbs.get(i).getIdx()) +"</div><br>");
...
//		대댓글을 입력하는 폼
		...
		//for문을 사용하여 대댓글 출력!
				List<RereplyBean> rrbs = rrdao.readRereply("jj",ridx);
...
<%
	out.print(bdao.categoryBoard("jj",bpage,"공지"));
	out.print(bdao.searchBoard("jj",bpage,standard,scontent));
%>
</table>
<!-- 검색 -->
...
<%
	int cidx=0;
	cidx = bdao.countsearchIdx("jj", standard, scontent);	
