...
<link rel="stylesheet" href="../css/list_kw.css">
<link rel="stylesheet" href="../css/read_kw.css">
<%
...
$(function(){
	...
	//현재 리스트 표시
	$("#tr<%=idx%>").css("background-color","#ebe5ce");
...
<table class="rf">
<%
	out.print(bdao.readContent("kw",idx));
%>
...
<!-- 버튼 모음 -->
<button onclick="location.href='kwBoard.jsp?page=<%=bpage%>'">목록가기</button>  
<%  if( bdao.getIdFromIdx("kw", idx).equals(id) ){%>
...
// 댓글
	List<ReplyBean> rbs = rdao.readReply("kw",idx);
	for(int i=rbs.size()-1; i>=0; i--){
		...
		out.println("<div class='rp_like'><a class='rlike'><img src='../images/smile.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyLike("kw", rbs.get(i).getIdx()) +" <a class='runlike' ><img src='../images/sad.png' class='sicon' width='25' height='25'></a> "+rdao.countReplyUnlike("kw", rbs.get(i).getIdx()) +"</div><br>");
...
// 		대댓글을 입력하는 폼
		...
		//for문을 사용하여 대댓글 출력!
				List<RereplyBean> rrbs = rrdao.readRereply("kw",ridx);
...
<!-- 페이지에 해당하는 목록 불러오기 -->
<table class="bd" id="kwBoard" >
<tr><th>글번호</th><th>분류</th><th>글제목</th><th>닉네임</th><th>조회수</th><th>작성날짜</th></tr>
<%
	if(bcategory!=null){
		out.print(bdao.categoryBoard("kw",bpage,bcategory));
	}else{
		out.print(bdao.readBoard("kw",bpage));	
	}
%>
</table>
...
<%
	int cidx=0;
	if(bcategory != null){
		cidx = bdao.countCategoryIdx("kw", bcategory);
	}else{
		cidx = bdao.countIdx("kw");	
	}
	...
	if(bpage<10){ // 10이상부터 형식이 바뀜
		if(page_size<=10){
			for(int i=page_size-1;i>=0;i--){
				if(bcategory != null){
					out.println("<a href='kwBoard.jsp?page="+pages.get(i)+"&category="+bcategory+"' id='p"+pages.get(i)+"'>"+pages.get(i)+"</a>");
				}else{
					out.println("<a href='kwBoard.jsp?page="+pages.get(i)+"' id='p"+pages.get(i)+"'>"+pages.get(i)+"</a>");
				}
			}
		}else{
			...
			if(bcategory != null){
				out.println("<a href='kwBoard.jsp?page="+(page_size)+"&category="+bcategory+"' id='p"+(page_size)+"'>"+(page_size)+"</a>");
			}else{
				out.println("<a href='kwBoard.jsp?page="+(page_size)+"' id='p"+(page_size)+"'>"+(page_size)+"</a>");
			}
			
			if(bcategory != null){
				out.println("<a href='kwBoard.jsp?page="+( ((bpage+5)>=page_size) ? page_size:bpage+5 )+"&category="+bcategory+"'>  다음-> </a>");
			}else{
				out.println("<a href='kwBoard.jsp?page="+( ((bpage+5)>=page_size) ? page_size:bpage+5 )+"'>  다음-> </a>");
			}
		...
		if(bcategory != null){
			out.println("<a href='kwBoard.jsp?page="+(bpage-5)+"&category="+bcategory+"'> <-이전  </a>");
		}else{
			out.println("<a href='kwBoard.jsp?page="+(bpage-5)+"'> <-이전  </a>");
		}
		if(bcategory != null){
			out.println("<a href='kwBoard.jsp?page=1&category="+bcategory+"' id='p1'>1</a>");
		}else{
			out.println("<a href='kwBoard.jsp?page=1 id='p1'>1</a>");
		}
